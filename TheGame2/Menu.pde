public class Menu{
  ...
  public Menu(String[] Tutorial){
    ...
    lvlLeft.resize(int(lvlLeft.get().width*4*barScale), int(lvlLeft.get().height*2*barScale));
    lvlRight.resize(int(lvlRight.get().width*4*barScale), int(lvlRight.get().height*2*barScale));
    ...
    lvlPointLeft.resize(int(lvlPointLeft.get().width*4*barScale), int(lvlPointLeft.get().height*2*barScale));
    lvlPointRight.resize(int(lvlPointRight.get().width*4*barScale), int(lvlPointRight.get().height*2*barScale));
    lvlPointMid.resize(int(lvlPointMid.get().width*2*barScale), int(lvlPointMid.get().height*2*barScale));
  ...
  void viewMenu(String hp, String lvl){ //displays the menu blocks and content of the pause menu
    ...
    for(int i = 0; i < healthBar.length; i++){
        ...
        image(lvlBar[i], panelLeft.center_x - MENU_MARGIN + i*lvlBar[i].width, panelLeft.center_y + 1.5*healthBar[0].height);
        
    }
    fill(#B24A16);
    textAlign(CENTER, CENTER);
    textSize(healthBar[0].height);
    text("HP: "+hp+"%", panelLeft.getLeft() + 1.5*MENU_MARGIN, panelLeft.center_y - healthBar[0].height/4);
    text("Lvl: "+lvl, panelLeft.getLeft() + 1.5*MENU_MARGIN, panelLeft.center_y + 1.5*healthBar[0].height - healthBar[0].height/4);
