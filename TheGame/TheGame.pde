...
final static float BLOCK_SIZE = 70.0;
...
public PGraphics blockBuffer; // new variable
...
void setup(){
 ...
 // player = new Sprite("data/zombie_stand.png", PLAYER_SCALE, 1.5 * BLOCK_SIZE, height - VERTICAL_MARGIN, 1); //Sprite("path", scale, xPos, yPos, frames)
  ...
  player = new Player( 1.5 * BLOCK_SIZE, 100);
  ...
  int mapHeight = mapFrame.length-1;
  int  mapWidth = split(mapFrame[0], ",").length -2;
  print(int(mapWidth*BLOCK_SIZE), int(mapHeight*BLOCK_SIZE));
  ...
  blockBuffer = createGraphics(int(mapWidth*BLOCK_SIZE), int(mapHeight*BLOCK_SIZE), JAVA2D); // initialize buffer
  blockBuffer.beginDraw();
  blockBuffer.background(255);
  for (Sprite block : blocks) {
    block.display(); // draw each block on the buffer
  }
  blockBuffer.endDraw();
  //noLoop();
...
void game_process(){
    ...
    player.update();
    //image(blockBuffer, 0, 0); // display block buffer
    /*for(Sprite block: blocks){
      block.display(); 
    }*/
    if(treasure){
